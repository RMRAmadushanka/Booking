...
const NavMobile: React.FC<NavMobileProps> = ({
    ...
    return (
      <ul className="nav-mobile-sub-menu pl-6 pb-1 text-base">
        {item.children?.map((i, index) => (
          <Disclosure key={(i.href || i.name) + index} as="li">
            <Link
              href={{
                pathname: i.href || undefined,
              }}
              className="flex px-4 text-[#0F172A] text-sm font-medium rounded-[var(--radius)] hover:bg-[#EFF6FF] hover:text-[#2563EB] transition-colors duration-150 mt-0.5"
            >
              <span
              ...
              {i.children && (
                ...
                >
                  ...
                  >
                    <ChevronDownIcon
                      className="ml-2 h-4 w-4 text-[#64748B]"
                      aria-hidden="true"
                    />
                  </Disclosure.Button>
                </span>
  ...
  const _renderItem = (item: NavItemType, index: number) => {
    return (
      ...
      >
        ...
        >
          <span
            className={`py-3 pr-3 ${!item.children ? "block w-full" : ""}`}
          >
            {item.name}
          </span>
          {item.children && (
            <span className="flex-1 flex" onClick={(e) => e.preventDefault()}>
              <Disclosure.Button
                as="span"
                className="py-3 flex items-center justify-end flex-1"
              >
                <ChevronDownIcon
                  className="ml-2 h-4 w-4 text-[#64748B]"
                  aria-hidden="true"
  ...
  return (
    <div className="overflow-y-auto w-full h-screen transition transform bg-white divide-y divide-[#E5E7EB]">
      {/* Header */}
